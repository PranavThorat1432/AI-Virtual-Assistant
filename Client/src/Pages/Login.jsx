...
const Login = () => {
  ...
  return (
    <div className='w-full h-screen bg-cover flex justify-center items-center' style={{backgroundImage: `url(${bg})`}}>
        <form className='w-[90%] h-[600px] max-w-[500px] bg-[#00000070] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-5 px-5' onSubmit={handleLogin}> 
          ...
          <button className='min-w-[150px] h-[60px] bg-white rounded-full cursor-pointer font-semibold text-[19px] mt-6' disabled={loading}>{loading ? 'Loading...' : 'Login'}</button>

          <p className='text-white text-[17px]'>Don't have an account? <span className='text-blue-400 cursor-pointer' onClick={() => navigate('/signup')}>Sign Up</span></p>
        </form>
    </div>
  )
}
